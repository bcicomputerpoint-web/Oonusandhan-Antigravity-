...
import { BookOpen, KeyRound, Mail, User, Building, AlertCircle } from 'lucide-react';
...
export default function Register() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const result = await register(name, email, password, role, institutionName || undefined);
    setLoading(false);
      ...
      <div className="w-full max-w-md bg-white rounded-3xl border border-slate-200 p-8 shadow-sm space-y-6">
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          <div className="space-y-1">
            <label className="text-xs font-bold text-slate-700 uppercase tracking-wider">{t('auth.fullName')}</label>
          ...
          <div className="space-y-1">
            <label className="text-xs font-bold text-slate-700 uppercase tracking-wider">{t('auth.roleSelect')}</label>
            <select
              value={role}
              onChange={(e) => setRole(e.target.value)}
              className="w-full px-3 py-2.5 bg-slate-50 border border-slate-200 rounded-xl focus:bg-white focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:border-transparent transition-all text-sm text-slate-800"
            >
              <option value="SCHOLAR">{t('roles.SCHOLAR')}</option>
          ...
          <div className="space-y-1">
            <label className="text-xs font-bold text-slate-700 uppercase tracking-wider">{t('auth.institutionName')}</label>
            <div className="relative">
              <input
                type="text"
        ...
        <button
          type="button"
          onClick={() => alert('Google authentication registration integration structure is ready!')}
